...
//#include "C:\Users\cohle\Desktop\CSG-Project\csg\csg_i.h"
...
struct sarray
{
  ...
  void clear() { if (p) { ::free(p); p = 0; n = 0; } }
  ...
  int indexof(T t)
  {
    for (UINT i = 0; i < n; i++) if (p[i] == t) return i;
    return -1;
  }
  void remove(T t)
  {
  ...
  {
    n--; for (UINT t = i; t < n; t++) p[t] = p[t + 1];
    //memcpy(p + i, p + (i + 1), (n - i - 1) * sizeof(T)); n--;
  }
  void insertat(UINT i, T t)
  {
    auto a = p ? _msize(p) / sizeof(T) : 0;
...
#define MO_DEPTHSTENCIL_COUNT		8
...
#define MO_DEPTHSTENCIL_REST  	0x00000007
...
#define MO_PSSHADER_TEXMASK			0x00050000
#define MO_PSSHADER_TEXTUREPTS  0x00060000
...
#define MO_VSSHADER_COUNT				3
#define MO_VSSHADER_WORLD				0x01000000
#define MO_VSSHADER_LIGHT				0x02000000
