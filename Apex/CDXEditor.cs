...
namespace Apex
{
  class CodeEditor : UserControl
  {
    void setfont()
    {
      var font = new System.Drawing.Font("Consolas", fsize[isize]);//, GraphicsUnit.Pixel);
      linehight = font.Height + 1;
      charwidth = (TextRenderer.MeasureText("00", font) - TextRenderer.MeasureText("0", font)).Width;
      this.font = font.ToHfont(); font.Dispose();
      sidedx = dpiscale(32); sidedu = dpiscale(12);
    }
    static readonly byte[] fsize = new byte[] { /*8, 9,*/ 10, 11, 12, 14, 16, 18, 20, 22 };
    internal int dpiscale(int i) { return linehight * i / 25; }// charwidth > 16 ? i * 2 : i; }// ; }
    IntPtr font; int isize = 1;
    int charwidth;
    int linehight;
    int sidedx, sidedu;
    ...
    public virtual int OnCommand(int id, object test)
    {
      switch (id)
      {
        ...
        case 2063: return OnToggleAll(test, 0);
        case 2064: return OnToggleAll(test, 1);
    ...
    int OnToggleAll(object test, int fl)
    {
      if (test != null) return 1;
      bool hide = false;
      if (fl == 1) hide = true; else foreach (var range in ranges) if (!range.hidden) { hide = true; break; }
      ranges = null;
      _updateranges();
    ...
    int OnToggle(object test)
    {
      ...
      for (int i = ranges.Count - 1, l = SelMax; i >= 0; i--)
      {
        ...
        {
          if (test != null) return 1;// range.hidden ? 3 : 1;
          range.hidden ^= true;
          _updatelineflags();
          _updatescrolls();
          Invalidate();
    ...
    {
      setfont(); _format(); _updateranges(); _updatelineflags();
      //ContextMenuStrip = new UIForm.ContextMenu { Tag = this };
    }
    protected override void OnHandleDestroyed(EventArgs e)
    {
      //ContextMenuStrip.Dispose(); 
      EndToolTip(); Native.DeleteObject(font);
    }
        ...
        for (int ta = 0, tb = 0, li = 0, ly = asy + dpiscale(4), xx = sidedx - dpiscale(12), irang = 0; ; tb++)
          if (tb == tn || s[tb] == '\n')
          {
            ...
            if ((lineflags[li] & 1) == 0)
            {
              if ((lineflags[li] & 4) != 0)
              {
                var plus = (lineflags[li] & 2) != 0; int yy = ly + (linehight >> 1) - dpiscale(7);
                if (ly + linehight > rc.Top)
                {
    ...
    protected override void WndProc(ref Message m)
    {
      switch (m.Msg)
      {
        ...
        case 0x020E: //WM_MOUSEWHEEL2
          {
            Invalidate(); var p = AutoScrollPosition; AutoScrollPosition =
              new Point(dpiscale(10) * (m.WParam.ToInt32() >> 16) / 20 - p.X, -p.Y);
          }
          break;
      }
      base.WndProc(ref m);
    ...
    protected override void OnMouseWheel(MouseEventArgs e)
    {
      ...
      {
        var i = Math.Max(0, Math.Min(fsize.Length - 1, isize + (e.Delta > 0 ? 1 : -1)));
        if (i == isize) return;
        var t1 = e.Location; var t2 = AutoScrollPosition; var t3 = AutoScrollMinSize;
        var x1 = (float)(t1.X - t2.X) / t3.Width;
        var y1 = (float)(t1.Y - t2.Y) / t3.Height;
        Native.DeleteObject(font); isize = i; setfont();
        Invalidate(); _updatescrolls(); var t4 = AutoScrollMinSize;
        AutoScrollPosition = new Point(t2.X == 0 ? 0 : (int)(x1 * t4.Width - t1.X), (int)(y1 * t4.Height - t1.Y));
        return;
      }
      var a = AutoScrollPosition; base.OnMouseWheel(e); var b = AutoScrollPosition;
    ...
    static int ccc(char c)
    {
      if (c == '(' || c == ')' || c == '{' || c == '}' || c == '[' || c == ']') return 3;
      if (char.IsLetterOrDigit(c) || c == '_') return 2;
      if (c > ' ') return 1;
      //if (c == 10) return -1;
      return 0;
    }
        ...
        case Keys.Left:
          {
            ...
            {
              var c = text[i]; var o = ccc(c);
              if (o == 0) for (; i != 0 && (o = ccc(c = text[i - 1])) == 0; i--) if (c == 10) break;
              if (c != 10) for (; i != 0 && ccc(text[i - 1]) == o; i--) ;
            }
        ...
        case Keys.Right:
          {
            ...
            {
              var c = text[i - 1]; var o = ccc(c); var n = text.Length;
              for (; i < n && ccc(c = text[i]) == o; i++) if (c == 10) break;
              if (o != 0) for (var t = i; i < n && ccc(c = text[i]) == 0; i++) if (c == 10) { i = t; break; }
            }
  ...
  {
    int dpiscale(int i) => i;// ((CodeEditor)Parent).dpiscale(i);
    ...
    protected override void OnHandleCreated(EventArgs e)
    {
      linedy = dpiscale(23); // Font.Height + dpiscale(6);
    ...
    internal void Format()
    {
      ...
      Width = midx + Math.Min(x2, Screen.GetBounds(this).Width - 100) + 4 + (Height < dy ? dpiscale(24) : 0); Height = Math.Min(dpiscale(400), dy);
    ...
    protected override void OnPaint(PaintEventArgs e)
    {
      ...
      for (int i = 0; i < items.Length; i++, yl += linedy)
      {
        ...
        if (item.pv != null && Type.GetTypeCode(item.pv.GetType()) == TypeCode.Object)
          TypeHelper.drawicon(g, dpiscale(2), yl + dpiscale(4), openitem == item ? 13 : 14, dpiscale(20));
        if (item == overitem) g.FillRectangle(Brushes.LightGray, dpiscale(17), yl, 4096, linedy);
        TypeHelper.drawicon(g, dpiscale(18), yl + dpiscale(1), item.icon, dpiscale(20));
        TextRenderer.DrawText(g, item.text, Font, new Point(dpiscale(37), yl + textofs), Color.Black); if (noeval) continue;
        TextRenderer.DrawText(g, item.sv.Length > 256 ? item.sv.Substring(0, 256) : item.sv, Font, new Point(midx + dpiscale(2), yl + textofs), Color.Black);
      }
    ...
    protected override void OnMouseDown(MouseEventArgs e)
    {
      ...
      drop.Location = PointToScreen(new Point(dpiscale(18), (i + 1) * linedy - 2 + AutoScrollPosition.Y));
  ...
  {
    internal static List<WeakReference> cache = new List<WeakReference>();
    ...
    static Bitmap icons;
    internal static void drawicon(Graphics g, int x, int y, int i, int d)
    {
      if (icons == null) { icons = Apex.Properties.Resource.typicons; icons.MakeTransparent(); }
